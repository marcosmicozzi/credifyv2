...
// Role Impact route - must come before /platform/:platform to avoid route collision
  ...
  if (params.metric === 'projects') {
    for (const up of userProjects) {
      ...
      if (up.u_role) {
        ...
        // Handle Supabase relation - roles can be an array or single object
        const rolesData = up.roles as unknown as { role_name: string; category: string | null } | { role_name: string; category: string | null }[] | null
